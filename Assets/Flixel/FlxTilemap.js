 ...
 class FlxTilemap extends FlxBasic
	{
		...
		public function FlxTilemap(MapData:String, TileGraphic:Object) { super(); init(MapData, TileGraphic, 1, 1); }
		public function FlxTilemap(MapData:String, TileGraphic:Object, CollisionIndex:uint) { super(); init(MapData, TileGraphic, CollisionIndex, 1); }
	    public function FlxTilemap(MapData:String, TileGraphic:Object, CollisionIndex:uint/*=1*/, DrawIndex:uint/*=1*/) { super(); init(MapData, TileGraphic, CollisionIndex, DrawIndex); }
		public function init(MapData:String, TileGraphic:Object, CollisionIndex:uint/*=1*/, DrawIndex:uint/*=1*/)
		{
			_ci = CollisionIndex;
			widthInTiles = 0;
			heightInTiles = 0;
...
//				_screenCols = widthInTiles;
		...
		/*override*/ public function render():void
		{
			...
			super.render();
			getScreenXY(_p);
			var tx:int = Mathf.Floor(-_p.x/_tileSize);
			var ty:int = Mathf.Floor(-_p.y/_tileSize);
			if(tx < 0) tx = 0;
...
//						FlxG.buffer.copyPixels(_pixels,_rects[cri],_p,null,null,true);
		...
		{
			var ix:uint = Mathf.Floor((Spr.x - x)/_tileSize);
			var iy:uint = Mathf.Floor((Spr.y - y)/_tileSize);
			var iw:uint = Mathf.Ceil(Spr.width/_tileSize)+1;
			var ih:uint = Mathf.Ceil(Spr.height/_tileSize)+1;
			var c:uint;
			for(var r:uint = 0; r < ih; r++)
			{
				...
				for(c = 0; c < iw; c++)
				{
					...
					var a:float = _data[(iy+r)*widthInTiles+ix+c];
					if(a >= _ci)
					{
