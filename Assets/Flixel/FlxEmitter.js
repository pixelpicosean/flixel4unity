...
class FlxEmitter extends FlxBasic
	{
		...
		public function FlxEmitter(X:Number, Y:Number, Width:uint, Height:uint, Sprites:FlxArray/*=null*/, Delay:Number/*=-1*/, MinVelocityX:Number/*=-100*/, MaxVelocityX:Number/*=100*/, MinVelocityY:Number/*=-100*/, MaxVelocityY:Number/*=100*/, MinRotation:Number/*=-360*/, MaxRotation:Number/*=360*/, Gravity:Number/*=500*/) {
			super();
	        init(X,Y,Width,Height,Sprites,Delay,MinVelocityX,MaxVelocityX,MinVelocityY,MaxVelocityY,
	            MinRotation,MaxRotation,Gravity,0,null,0,false,null);
		}
	...
	public function FlxEmitter(X:Number, Y:Number, Width:uint, Height:uint, Sprites:FlxArray/*=null*/, Delay:Number/*=-1*/, MinVelocityX:Number/*=-100*/, MaxVelocityX:Number/*=100*/, MinVelocityY:Number/*=-100*/, MaxVelocityY:Number/*=100*/, MinRotation:Number/*=-360*/, MaxRotation:Number/*=360*/, Gravity:Number/*=500*/, Drag:Number/*=0*/, Graphics/*:Class*/ /*=null*/, Quantity:uint/*=0*/, Multiple:boolean/*=false*/, Parent:FlxLayer/*=null*/) {
	    super();
		init(X,Y,Width,Height,Sprites,Delay,MinVelocityX,MaxVelocityX,MinVelocityY,MaxVelocityY,
		    MinRotation,MaxRotation,Gravity,Drag,Graphics,Quantity,Multiple,Parent);
    }

		public function init(X:Number, Y:Number, Width:uint, Height:uint, Sprites:FlxArray/*=null*/, Delay:Number/*=-1*/, MinVelocityX:Number/*=-100*/, MaxVelocityX:Number/*=100*/, MinVelocityY:Number/*=-100*/, MaxVelocityY:Number/*=100*/, MinRotation:Number/*=-360*/, MaxRotation:Number/*=360*/, Gravity:Number/*=500*/, Drag:Number/*=0*/, Graphics/*:Class*/ /*=null*/, Quantity:uint/*=0*/, Multiple:boolean/*=false*/, Parent:FlxLayer/*=null*/)
		{
			visible = false;
			x = X;
			y = Y;
			...
			if(Graphics != null)
			{
				...
				for(i = 0; i < Quantity; i++)
				{
                    ...
                    // (_sprites[_sprites.length-1] as FlxSprite)._sprite.SetColor(new Color(0,1,0,1));
				}
		...
		/*override*/ public function update():void
		{
			...
			if(_delay < 0)
			{
				...
				if(!(_sprites[0] as FlxSprite).exists)
				{
		...
		public function emit():void
		{
			...
			s.exists = true;
			s.x = x - (s.width>>1);
			if(width != 0) s.x += Random.value*width;
			s.y = y - (s.height>>1);
			if(height != 0) s.y += Random.value*height;
			s.velocity.x = minVelocity.x;
			if(minVelocity.x != maxVelocity.x) s.velocity.x += Random.value*(maxVelocity.x-minVelocity.x);
			s.velocity.y = minVelocity.y;
			if(minVelocity.y != maxVelocity.y) s.velocity.y += Random.value*(maxVelocity.y-minVelocity.y);
			s.acceleration.y = _gravity;
			s.angularVelocity = _minRotation;
			if(_minRotation != _maxRotation) s.angularVelocity += Random.value*(_maxRotation-_minRotation);
			if(s.angularVelocity != 0) s.angle = Random.value*360-180;
			s.drag.x = _drag;
			s.drag.y = _drag;
			_particle++;
		...
		/*override*/ public function kill():void
		{
			active = false;
			for(var i:uint = 0; i < _sprites.length; i++)
				(_sprites[i] as FlxSprite).exists = false;
		}
	}
//}
